...
| US-RF-021 Planning page MVP (Stop + Option model) | Stop + Option model: domain validation, pin / unpin use cases, manual / AI / Google options, drag-drop reorder, route overview placeholder, AI suggestions panel, 4-role permission (owner / planner / member / viewer) | yes | yes (schema) | not run | yes (build) | implemented | 16 domain + 16 use-case + 11 workspace-state + 2 permissions = 45 new tests pass; schema-contract 10 pass (covers `trip_stops` + `stop_options`); `pnpm build` pass; route `/trips/[tripId]/planning-v2` 7.87 kB + 129 kB shared; 252/252 tests pass total; `pnpm lint` clean; story packet `docs/stories/epics/E04-rideflow-v1/US-RF-021/US-RF-021-planning-page-mvp.md`; spec `docs/specs/004-planning-mvp/spec.md`; decision `docs/decisions/0009-stop-option-model.md` |
